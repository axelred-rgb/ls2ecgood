...
class RegistrationController extends Controller {
    ...
    public static function provideemailAction(){
        ...
        return array('success' => true,
            'user' => $user,
            'redirect' => route('enrolled?id='.$idcourse),
            'detail' => t("adresse mail ajouté"));
    }
...
//        global $appuser;
        ...
        if ($user->getIsActivated()==1)
            return [
                "success" => true,
                "detail" => "Votre compte est déja activé",
                "redirect" => $route,
            ];

        else {
            $code = md5($_POST["activationcode"]);

            if ($code == $user->getActivationcode()) {
            //if (substr($code, 0, 5) == $appuser->getActivationcode()) {

                $appuser->setIsActivated(1);
        ...
        return [
            "success" => false,
            "error" => ["my error" =>"Activation code not valide. ensure that there is no space"]
        ];
...
//        global $appuser;
        ...
        $user = User::where('this.email',$_POST["email"])->__getOne();;
        ...
        if ($user->getId()){
            ...
            $user->setIspassforgot(1);
            $id = $user->__update();

            $data = [
                "activation_link" => route('resetpassword').'?vld='.$hashcode.'&u_id='.$user->getId(),
                "username" => $user->getUsername(),
                "activation_code" => $code,
            ];
            //$emailmodel->sendMail($data);
            Reportingmodel::init("resetpassword")
                ->addReceiver($user->getEmail(), $user->getUsername())
                ->sendMail($data);

            return [
                "success" => true,
                "detail" => ["detail" => "Un mail vous à été envoyé"],
                "redirect" => "confirmresetpassword",
            ];
        }
        else {
            ...
            return [
                "success" => false,
                "error" => ["error" => "Email n'existe pas"],
            ];

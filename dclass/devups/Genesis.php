...
//            dv_dump($rigths);
    ...
    public static function render($view, $data = [])
    {
        ...
        if($view !== 'error' && $view !== 'erroraccess' && $view !== 'errorserveur'){
            //$app->{$function}($paramvalues);
            // Obtenir le protocole (HTTP ou HTTPS)
            $protocol = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https://' : 'http://';

            // Obtenir le nom du domaine (par exemple, www.example.com)
            $domain = $_SERVER['HTTP_HOST'];

            // Obtenir l'URI de la requête (la partie après le nom de domaine)
            $uri = $_SERVER['REQUEST_URI'];

            // L'URL complète est la combinaison du protocole, du nom de domaine et de l'URI
            $url = $protocol . $domain . $uri;

            // Afficher l'URL complète

            $_SESSION['ls2ec_last_url'] = $url;

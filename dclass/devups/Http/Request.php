...
class Request
{
    ...
    function __construct($default_path = 'hello')
    {
        ...
        if(!isset($_SESSION["last_lang"]))
            $_SESSION["last_lang"] = __lang;
    ...
    public static function Route($app, $path)
    {
        ...
        if(Request::get("lang") != $_SESSION["last_lang"]) {
            $env = str_replace(__server, "", __env);
            $url = str_replace($env.$_SESSION["last_lang"], __env.Request::get("lang").'', $_SESSION["last_url"]);
            $_SESSION["last_lang"] = Request::get("lang");
            redirect($url);
        }
        $_SESSION["last_url"] = self::$uri;
        $array = explode("-", $path);
        $function = "";
...
//            $app->{$function}();
     ...
     * @return \stdClass
     */
    public function json($option = false){
        return json_decode($this->_response, $option);
    }

    ...
    <div class="tab-pane fade active show" id="pills-mylabs" role="tabpanel" aria-labelledby="pills-mylabs-tab">
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <a href="{{route('createPost')}}" class="btn btn-md btn-success pull-right"> <i class="fa fa-plus"></i> @tt("New Post")</a>
            </div>
        </div>
                ...
                <div class="box">
                    ...
                    <div class="row g-0">
                        ...
                        <div class="col-md-8 col-12">
                            <div class="box-body">
                                <h4><a href="{{route('blog')}}?id={{$articles->getId()}}" id="title{{$articles->getId()}}">{{$articles->getTitle()}}</a></h4>
                                ...
                                <div class="flexbox align-items-center mt-3">
                                    <a class="btn btn-sm btn-primary" href="{{route('blog')}}?id={{$articles->getId()}}"> <i class="fa fa-eye"></i> @tt("Read more")</a>
                                    <a class="btn btn-sm btn-warning" href="{{route('editblog')}}?id={{$articles->getId()}}"> <i class="fa fa-edit"></i> @tt("Update Post")</a>
                                    <button class="btn btn-sm btn-danger" onclick="getDataDelete('{{$articles->getId()}}')" id="buttonDelete"  style="background-color:#e9182f;border-color:#e9182f"> <i class="fa fa-trash"></i> @tt("Delete Post")</button>
                                    <a class="btn btn-sm btn-info" href="{{route('downloadPdf')}}?id={{$articles->getId()}}" style="background-color:#0077b5;border-color:#0077b5"> <i class="fa fa-file"></i> @tt("PDF")</a>
...
@section("jsimport")
	...
	<script>
        ...
        function getDataDelete(post){
            ...
            $.jAlert({
                'title':'',
                'content': "@tt('Do you want too delete post') "+title+" ?.<br><strong style='color:#e9182f'>WARNING!! this action is irreversible</strong>",
                'theme': 'blue',
                'btns': [
                    {'text':'@tt("Yes")', 'closeAlert':false, 'onClick': function(){app.deletePost(this,id);return false; }},

            ...
            <table class="table no-border">
                ...
                @foreach(array_reverse($codes) as $code)
                    ...
                    @if($isOk)
                        ...
                        <tr style="cursor: pointer;" onclick="window.location.href = '{{route('affiliationtransactionbysession')}}?code={{$code->getCodepromo()->getCode()}}&session={{$sessionpaiement->getId()}}'">

                            <td>{{$code->getCodepromo()->getCode()}}</td>
                            <td>

    ...
    <div class="tab-pane fade active show" id="pills-mylabs" role="tabpanel" aria-labelledby="pills-mylabs-tab">
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <a href="{{route('createExercise')}}" class="btn btn-md btn-success pull-right"> <i class="fa fa-plus"></i> @tt("New Exercise")</a>
            </div>
        </div>
        ...
        @foreach(array_reverse($exercises) as $exercise)
            <div class="col-lg-12 col-12">
                <div class="box">
                    ...
                    <div class="row g-0">
                        <div class="col-md-4 col-12 bg-img h-md-auto h-250" style="background-image: url({{$exercise->getImage()}})"></div>
                        <div class="col-md-8 col-12">
                            <div class="box-body">
                                <h4 id="title{{$exercise->getId()}}">{{$exercise->getTitle()}}</h4>
                                <div class="d-flex mb-10">
                                    <div class="me-10">
                                        <i class="fa fa-book-open me-5"></i> {{$exercise->getCourses()->getName()}}
                                    </div>
                                </div>


                                <div class="flexbox align-items-center mt-3">
                                    <a class="btn btn-sm btn-warning" href="{{route('editexercise')}}?id={{$exercise->getId()}}"> <i class="fa fa-edit"></i> @tt("Update Exercise")</a>
                                    <a class="btn btn-sm btn-success" href="{{route('detailexercise')}}?id={{$exercise->getId()}}"> <i class="fa fa-eye"></i> @tt("See Exercise")</a>
                                    <button class="btn btn-sm btn-danger" onclick="getDataDelete('{{$exercise->getId()}}')" id="buttonDelete"  style="background-color:#e9182f;border-color:#e9182f"> <i class="fa fa-trash"></i> @tt("Delete Exercise")</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
@section("jsimport")
    ...
    <script>
        ...
        function getDataDelete(post){
            ...
            $.jAlert({
                'title':'',
                'content': "@tt('Do you want too delete exercise') "+title+" ?.<br><strong style='color:#e9182f'>@tt('WARNING!! this action is irreversible')</strong>",
                'theme': 'blue',
                'btns': [

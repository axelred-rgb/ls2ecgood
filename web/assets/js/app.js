...
newsletter: function (event) {
        ...
        Drequest.api("user.resetaccount")
            ...
            if (response.success) {
                ...
                $('.forgot-error').html(`<div class="alert alert-success alert-dismissable">
                                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×
                                    </button> Un mail vous a été envoyé afin de finaliser la procédure
                                </div>
                                `);
                //setTimeout(() => window.location.reload(), 3)
                setTimeout(function () {
    ...
    paymentTokenFinal: function(event){
        ...
        $("#cinetpaycontent").attr('hidden','hidden');
        var packtoken = $("#purchase-token").find("select[name=packtoken]").val();
        var quantity = $("#purchase-token").find("input[name=quantity]").val();
            ...
            Drequest.api("get.packTokenInformation")
                ...
                .post(function (response) {
                    if(response.success){
                        ...
                        $("#cinetpaycontent").removeAttr('hidden');
                        let route2 = '';
                        $('#cinetpay').attr('onclick',`checkout('`+route2+`',`+parseInt(total)+`,'`+devise+`','Pack "`+response.qte+`" Token"','Pack "`+response.qte+`" Token"','`+surname+`','`+email+`','`+phonenumber+`','`+country+`','`+countryname+`',`+function(){app.saveTokenBuy()}+`)`);
    ...
    upgradepassword: function(event){
        model.addLoader($(event));
    ...
    async addtocart(el){
        ...
        model.addLoader($(el));
        // await tt('Opération en cours').then(function (data) {
        //     $('#addcart').html(data.data);
        // }).catch(function (err) {
        //     console.log(err);
        // });

        $('#addcart').html('Opération en cours');
        $('#message').html('');
        var formdata = new FormData();
        let productid;
        let qte;
        if($(el).attr('data-id') !== undefined){
            ...
            productid = $('#produitsid').val();
            qte = $('#qte').val();
        }

        formdata.append('produitsid', productid);
        formdata.append('qte', qte);
        Drequest.api("add.tocart")
            .data(formdata)
            .post(async function (response) {
                model.removeLoader();
                console.log(response);
                // await tt('Ajouter au panier').then(function (data) {
                //     $('#addcart').html(data.data);
                // }).catch(function (err) {
                //     console.log(err);
                // });
                //$('#addcart').html('Ajouter au panier');
                if (response.success) {

                    $(el).removeAttr('onclick');
                    $(el).html('Mon panier');
                    $(el).attr('href',__env+'panier');

                    alert(response.detail);
                    $('#dot').html(response.panier);

                }
                else{
                    model.removeLoader();

                    $('#message').html('<div class="error">'+response.detail+'</div>');
                }
            })
    },
